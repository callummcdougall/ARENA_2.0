...
def induction_score_hook(
    ...
    induction_score = einops.reduce(
	    induction_stripe,
	    "n_batches n_heads seq_len -> n_heads",
	    reduction="mean",
	)
    induction_score_store[hook.layer(), :] = induction_score
...
def logit_attribution(
    ...
    direct_contrib = einops.einsum(
	    embed[:-1], W_U_correct_tokens,
	    "seq d_model, d_model seq -> seq",
    )
    direct_contrib = einops.repeat(
	    direct_contrib,
        "seq -> seq one",
        one=1,
    )
    l1_contrib = einops.einsum(
	    l1_results[:-1], W_U_correct_tokens,
	    "seq nheads d_model, d_model seq -> seq nheads",
    )
    l2_contrib = einops.einsum(
	    l2_results[:-1], W_U_correct_tokens,
	    "seq nheads d_model, d_model seq -> seq nheads",
    )
    component_contrib = t.cat([direct_contrib, l1_contrib, l2_contrib], dim=-1)
...
def head_ablation_hook(
    ...
    result = t.tensor(attn_result)
    result[:, :, head_index_to_ablate, :].zero_()
    return result
...
def decompose_qk_input(cache: ActivationCache) -> t.Tensor:
    ...
    e = einops.repeat(
	    cache["embed"],
	    "seq_len d_model -> n_components seq_len d_model",
	    n_components=1,
    )
    pe = einops.repeat(
	    cache["pos_embed"],
    ...
    x = einops.rearrange(
	    cache["result", 0],
	    "seq_len n_components d_model -> n_components seq_len d_model",
    )
...
def decompose_q(decomposed_qk_input: t.Tensor, ind_head_index: int) -> t.Tensor:
    ...
    result = einops.einsum(
        decomposed_qk_input, model.W_Q[1, ind_head_index],
        "n_components seq_len d_model, d_model d_head -> n_components seq_len d_head"
    )
    return result
...
def decompose_k(decomposed_qk_input: t.Tensor, ind_head_index: int) -> t.Tensor:
    ...
    '''
    result = einops.einsum(
        decomposed_qk_input, model.W_K[1, ind_head_index],
        "n_components seq_len d_model, d_model d_head -> n_components seq_len d_head"
    )
    return result
...
def decompose_attn_scores(decomposed_q: t.Tensor, decomposed_k: t.Tensor) -> t.Tensor:
    ...
    return einops.einsum(
        decomposed_q, decomposed_k,
        "n_q_components seq_len_q d_head, n_k_components seq_len_k d_head -> n_q_components n_k_components seq_len_q seq_len_k",
    )
...
if MAIN:
    ...
    # # YOUR CODE HERE - fill in each tensor in the dictionary, by looping over W_A and W_B from layers 0 and 1
    # for l0_head_idx in range(model.cfg.n_heads):
    #       for l1_head_idx in range(model.cfg.n_heads):
    #             composition_scores["Q"][l0_head_idx, l1_head_idx] = get_comp_score(
    #                 W_A=model.W_Q[0, l0_head_idx] @ model.W_K[0, l0_head_idx].T,
    #                 W_B=model.W_V[1, l1_head_idx] @ model.W_O[1, l1_head_idx],
    #             )
    #             composition_scores["K"][l0_head_idx, l1_head_idx] = get_comp_score(
    #                 W_A=model.W_K[0, l0_head_idx] @ model.W_Q[0, l0_head_idx].T,
    #                 W_B=model.W_V[1, l1_head_idx] @ model.W_O[1, l1_head_idx],
    #             )
    #             composition_scores["V"][l0_head_idx, l1_head_idx] = get_comp_score(
    #                 W_A=model.W_V[0, l0_head_idx] @ model.W_O[0, l0_head_idx],
    #                 W_B=model.W_V[1, l1_head_idx] @ model.W_O[1, l1_head_idx],
    #             )
...
#     Write a function which generates a single composition score for random matrices
#     '''
#     W = t.empty((4, model.cfg.d_model, model.cfg.d_head))
#     nn.init.kaiming_uniform_(W, a=np.sqrt(5))
#     return get_comp_score(W[0] @ W[1].T, W[2] @ W[3].T)
