...
class Model(nn.Module):
    ...
    def __init__(
        ...
        self.W = t.nn.Parameter(t.zeros(config.n_instances, config.n_hidden, config.n_features).to(device))
        t.nn.init.xavier_normal_(self.W)
        self.b_final = t.nn.Parameter(t.zeros(config.n_instances, config.n_features).to(device))
...
def generate_correlated_batch(self: Model, n_batch: int) -> Float[Tensor, "n_batch instances fetures"]:
    ...
    feat = t.rand((n_batch, n_instances, n_uncorrelated), device=self.W.device)
    feat_seeds = t.rand((n_batch, n_instances, n_uncorrelated), device=self.W.device)
    feat_is_present = feat_seeds <= self.feature_probability
    ...
    feat = t.rand((n_batch, n_instances, n_correlated_pairs*2), device=self.W.device)
    feat_seeds = t.rand((n_batch, n_instances, n_correlated_pairs), device=self.W.device)
    feat_seeds = einops.repeat(feat_seeds, 'batch instances pairs -> batch instances (pairs 2)')
    feat_is_present = feat_seeds <= self.feature_probability
    ...
    # print(f"{n_anticorrelated_pairs=}")
    feat = t.rand((n_batch, n_instances, n_anticorrelated_pairs, 2), device=self.W.device)
    feat_seeds = t.rand((n_batch, n_instances, n_anticorrelated_pairs), device=self.W.device)
    # Clone feat_seeds after `repeat` because we assign to individual elements later
    feat_seeds = einops.repeat(feat_seeds, 'batch instances pairs -> batch instances pairs 2').clone()
    # print(f"{feat_seeds.shape=}")
    indices = t.randint(2, size=feat_seeds.shape[:-1])
    idx = t.meshgrid(*[t.arange(s) for s in feat_seeds.shape[:-1]])
    feat_seeds[idx + (indices,)] = 3.
    # print(f"{feat_seeds.shape=}")
    feature_probability = einops.repeat(self.feature_probability, "instance pair -> instance pair in_pair", pair=1, in_pair=1)
    feat_is_present = feat_seeds <= 2 * feature_probability
    # print(f"{feat_is_present.shape=}")
    # each pair has total prob 2 * p so individual features have probability p
    batch_anticorrelated = t.where(
        feat_is_present,
        feat,
        t.zeros((), device=self.W.device)
    )
    batch_anticorrelated = einops.rearrange(batch_anticorrelated, "... pairs in_pair -> ... (pairs in_pair)")

    # print(f"{batch_correlated.shape=}")
    # print(f"{batch_anticorrelated.shape=}")
    # print(f"{batch_uncorrelated.shape=}")
