...
class Model(nn.Module):
    ...
    def __init__(
        ...
        self.W = nn.Parameter(t.zeros((config.n_instances, config.n_hidden,
                                       config.n_features), device=device))
        nn.init.xavier_normal_(self.W)
        self.b_final = nn.Parameter(t.zeros((config.n_instances, config.n_features),
                                            device=device))
    ...
    def forward(
        ...
        out = einops.einsum(features, self.W,
                            '... instances features, instances hidden features -> ... instances hidden')
        out = einops.einsum(out, self.W,
