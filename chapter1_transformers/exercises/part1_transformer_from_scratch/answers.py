...
class Attention(nn.Module):
    ...
    def __init__(self, cfg: Config, cached=False):
        super().__init__()
        self.cfg = cfg
        ...
        self.cached = cfg.cached
        if self.cached:
            self.old_keys = None
            self.old_values = None
            self.old_res_stream = None
    ...
    def forward(
        ...
        print(self.cached)
        if self.cached and self.old_keys is not None:
            print("retrieving from cache")
            new_query = einops.einsum(
                    self.W_Q,
                    normalized_resid_pre[:,-1:],
            ...
            keys = t.cat((self.old_keys, new_key), dim=1)

            new_attn_score_row = einops.einsum(
                keys,
                new_query,
                "batch seq_len_k heads d_head, \
                                        batch seq_len_q heads d_head -> batch heads seq_len_q seq_len_k",
            ) / math.sqrt(self.cfg.d_head)
            
            new_attn_prob_row = t.softmax(new_attn_score_row, dim=-1)
            ...
            values = t.cat((self.old_values, new_value), dim=1)

            new_z = einops.einsum(
                new_attn_prob_row, # seq_len_q = 1
                values,
            ...
            new_z = einops.einsum(
                    new_z,
                    self.W_O,
                    "batch seq_len_q heads d_head, heads d_head d_model \
                                -> batch seq_len_q d_model",
                ) + self.b_O

            new_res_stream = t.cat((self.old_res_stream, new_z), dim=1)

            # update cache
            self.old_keys = keys
            self.old_values = values
            self.old_res_stream = new_res_stream

            return new_res_stream
            ...
            attn_probs = t.softmax(
                attn_scores, dim=-1
            )  # want key rows to be prob distributions
            # rows of attention matrix will add up to 1

            z_values = einops.einsum(
                attn_probs,
                values,
                "batch heads seq_len_q seq_len_k, \
                                    batch seq_len_k heads d_head -> batch seq_len_q heads d_head",
            ...
            z_values = (
                ...
                + self.b_O
            )  # sum over heads, matmul over d_head

            if self.cached:
                print("making new cache")
                self.old_keys = keys
                self.old_values = values
                self.old_res_stream = z_values
    ...
    def forward(
        ...
        hidden = (
            einops.einsum(
                ...
                self.W_in,
                "batch seq_len d_model, d_model d_mlp \
                               -> batch seq_len d_mlp",
            )
            + self.b_in
        )
...
class TransformerBlock(nn.Module):
    def __init__(self, cfg: Config, cached=False):
        ...
        self.attn = Attention(cfg, cached=cached)
...
class DemoTransformer(nn.Module):
    def __init__(self, cfg: Config, cached=False):
        ...
        self.blocks = nn.ModuleList(
            [TransformerBlock(cfg, cached=cached) for _ in range(cfg.n_layers)]
        )
...
class LitTransformer(pl.LightningModule):
    ...
    def training_step(self, batch: Dict[str, Tensor], batch_idx: int) -> Float[Tensor, ""]:
        ...
        logits = model(tokens)
        logprobs = get_log_probs(logits, tokens) # "batch posn-1"
        loss = -t.mean(logprobs)
        self.log('train_loss', loss)
        return loss

    def configure_optimizers(self) -> Any:
        optimizer = t.optim.AdamW(params=model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
        return optimizer
...
if MAIN:
    ...
    print(f"Trying to start project with name {args.log_name}")
    logger = WandbLogger(save_dir=args.log_dir, project=args.log_name, name=args.run_name, entity="tkwa")
...
class TransformerSampler:
    ...
    def sample(self, prompt: str, max_tokens_generated=100, verbose=False, **kwargs):
        ...
        '''
        tokens = t.tensor(self.tokenizer.encode(prompt)).unsqueeze(0)
        prompt_length = tokens.shape[1]
        while tokens.shape[1] < prompt_length + max_tokens_generated and tokens[0, -1] != self.tokenizer.eos_token_id:
            logits = self.model(tokens)[:, -1]
            next_token = t.tensor([[self.sample_next_token(tokens[0], logits, **kwargs)]])
            print(next_token)
            print(f"{tokens.shape=}")
            print(f"{next_token.shape=}")
            tokens = t.cat((tokens, t.tensor([[next_token]])), dim=1).clone()
            print(tokens)
            print(tokens.shape)
    ...
    def greedy_search(logits: Float[Tensor, "d_vocab"]) -> int:
        ...
        '''
        assert len(logits.shape) == 1 or len(logits.shape) == 2 and logits.shape[0] == 1
    ...
    def sample_basic(logits: Float[Tensor, "d_vocab"]) -> int:
        ...
        '''
        dist = t.distributions.categorical.Categorical(logits)
        ret = dist.sample().item()
        print(ret)
