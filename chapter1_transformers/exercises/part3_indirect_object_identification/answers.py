...
example_prompt = "After Mary, John and Mary went to the store, John gave a bottle of milk to"
example_answer = " Mary"
...
def logits_to_ave_logit_diff(
    ...
    wrong_tokens = answer_tokens[:, 1]

    logits = logits[:, -1, :]  # last logit only

    correct_logits = logits.gather(1, correct_tokens.unsqueeze(1)).squeeze()
    wrong_logits = logits.gather(1, wrong_tokens.unsqueeze(1)).squeeze()

    print(correct_logits)
    print(wrong_logits)

    diff = correct_logits - wrong_logits
...
# %%
print(cache.apply_ln_to_stack.__doc__)
# %%
...
def ioi_metric(
    ...
    # logit_diff == corrupted_logit_diff --> 0
    # logit_diff == clean_logit_diff --> 1
    # clean_logit_diff > corrupted_logit_diff
    change = logit_diff - corrupted_logit_diff
    max_change = clean_logit_diff - corrupted_logit_diff
...
# %%
print(model.run_with_hooks.__doc__)
# %%
def patch_residual_component(
    ...
    '''
    clean_val = clean_cache[hook.name][:, pos, :]
    corrupted_residual_component[:, pos, :] = clean_val
...
def get_act_patch_resid_pre(
    ...
    for layer_ix in range(model.cfg.n_layers):
        ...
        for pos in range(corrupted_tokens.shape[-1]):
            hook_name = utils.get_act_name("resid_pre", layer_ix)
            hook_fn = partial(patch_residual_component, pos=pos, clean_cache=clean_cache)
            logits = model.run_with_hooks(corrupted_tokens, fwd_hooks = [(hook_name, hook_fn)], return_type="logits")
            layer_out.append(patching_metric(logits))
        out.append(layer_out)
    
    return t.tensor(out).to(device)
...
def patch_head_vector(
    ...
    '''
    clean_val = clean_cache[hook.name][:, :, head_index, :]
    corrupted_head_vector[:, :, head_index, :] = clean_val
...
def get_act_patch_attn_head_out_all_pos(
    ...
    for layer_ix in range(model.cfg.n_layers):
        layer_out = []
        for head_ix in range(model.cfg.n_heads):
            hook_name = utils.get_act_name("z", layer_ix)
            hook_fn = partial(patch_head_vector, head_index=head_ix, clean_cache=clean_cache)
            logits = model.run_with_hooks(corrupted_tokens, fwd_hooks = [(hook_name, hook_fn)], return_type="logits")
            layer_out.append(patching_metric(logits))
        out.append(layer_out)
