...
def logits_to_ave_logit_diff(
    ...
    '''
    B, _ = answer_tokens.shape
    final_logit = logits[:,-1,:]
    answer_logits = t.gather(final_logit, dim=-1, index=answer_tokens)
    correct, incorrect = answer_logits.unbind(dim=-1)
    diffs = correct - incorrect
    if per_prompt == False:
        diffs = diffs.mean()
    return diffs
...
def residual_stack_to_logit_diff(
    ...
    '''
    B, _ = logit_diff_directions.shape
    
    scaled_final_token_residual_stack = cache.apply_ln_to_stack(residual_stack, layer=-1, pos_slice=-1)

    average_logit_diff = einops.einsum(
        scaled_final_token_residual_stack, logit_diff_directions,
        '... batch d_model, batch d_model -> ...'
    ) / B
...
def ioi_metric(
    ...
    '''
    diffs = logits_to_ave_logit_diff(logits, answer_tokens)
    return (diffs - corrupted_logit_diff) / (clean_logit_diff - corrupted_logit_diff)
...
def get_act_patch_resid_pre(
    ...
    '''
    B, P = corrupted_tokens.shape
    result = t.empty(model.cfg.n_layers, P).to(device)
    model.reset_hooks()

    for layer in range(model.cfg.n_layers):
        for pos in range(P):

            # define a temp hook function from patch_residual_component
            # you don't specify the tensor or hook arguments here
            # because it's a function, so it takes those in later!
...
def get_act_patch_block_every(
    ...
    '''
    B, P = corrupted_tokens.shape
    result = t.empty(3, model.cfg.n_layers, P).to(device)
    model.reset_hooks()
    ...
    for layer in range(model.cfg.n_layers):
        for pos in range(P):
            ...
            logits_resid_pre = model.run_with_hooks(
                corrupted_tokens, 
                fwd_hooks = [(utils.get_act_name('resid_pre', layer), temp_hook_fn)]
            )

            result[0,layer, pos] = patching_metric(logits_resid_pre)

            logits_attn_out = model.run_with_hooks(
                corrupted_tokens, 
                fwd_hooks = [(utils.get_act_name('attn_out', layer), temp_hook_fn)]
            )

            result[1,layer, pos] = patching_metric(logits_attn_out)

            logits_mlp_out = model.run_with_hooks(
                corrupted_tokens, 
                fwd_hooks = [(utils.get_act_name('mlp_out', layer), temp_hook_fn)]
            )

            result[2,layer, pos] = patching_metric(logits_mlp_out)
...
def get_act_patch_attn_head_out_all_pos(
    ...
    called on the model's logit output.
    '''
    result = t.empty(model.cfg.n_layers, model.cfg.n_heads).to(device)
    model.reset_hooks()
...
def get_act_patch_attn_head_all_pos_every(
    ...
    called on the model's logit output.
    '''
    result = t.empty(5, model.cfg.n_layers, model.cfg.n_heads).to(device)
    model.reset_hooks()
    ...
    for layer in tqdm(range(model.cfg.n_layers)):
        for head in range(model.cfg.n_heads):
            ...
            # output
            logits_output = model.run_with_hooks(
                corrupted_tokens,
                fwd_hooks=[(utils.get_act_name('z', layer),hook_fn)]
                )
            result[0, layer, head] = patching_metric(logits_output)

            # queries
            logits_q = model.run_with_hooks(
                corrupted_tokens,
                fwd_hooks=[(utils.get_act_name('q', layer),hook_fn)]
                )
            result[1, layer, head] = patching_metric(logits_q)

            # keys
            logits_k = model.run_with_hooks(
                corrupted_tokens,
                fwd_hooks=[(utils.get_act_name('k', layer),hook_fn)]
                )
            result[2, layer, head] = patching_metric(logits_k)

            # values
            logits_v = model.run_with_hooks(
                corrupted_tokens,
                fwd_hooks=[(utils.get_act_name('v', layer),hook_fn)]
                )
            result[3, layer, head] = patching_metric(logits_v)

            pattern_hook_fn = partial(patch_attn_patterns, head_index=head, clean_cache=clean_cache)
            # pattern
            logits_pattern = model.run_with_hooks(
                corrupted_tokens,
                fwd_hooks=[(utils.get_act_name('pattern', layer),pattern_hook_fn)],
                return_type='logits'
                )
            result[4, layer, head] = patching_metric(logits_pattern)
    return result
...
def patch_head(
    ...
    # patch head vector with corrupted
    # patch all other heads with clean?
    head_vector[:,:,head_idx] = cache[hook.name][:,:,head_idx]
    return head_vector
...
def get_path_patch_head_to_final_resid_post(
    ...
    model.reset_hooks()
    # cache the head outputs for a run on clean and corrupted inputs
    
    _, clean_cache = model.run_with_cache(orig_dataset.toks, names_filter = lambda name: name.endswith('z'))
    _, corrupted_cache = model.run_with_cache(new_dataset.toks, names_filter = lambda name: name.endswith('z'))

    result = t.empty(model.cfg.n_layers, model.cfg.n_heads).to(device)
    
    print(f'{orig_dataset.toks.shape=}')
            ...
            for l in range(layer, model.cfg.n_layers):
                for h in range(model.cfg.n_heads):
                    if l == layer and h == head:
                        # patch head with corrupted
                        patch_hook_fn = partial(patch_head, head_idx =h, cache=corrupted_cache)
                        model.add_hook(utils.get_act_name('z', l), patch_hook_fn)
                        #fwd_hooks.append((utils.get_act_name('z', l), patch_hook_fn))
                    else:
                        # freeze head, i.e. patch with clean
                        freeze_hook_fn = partial(patch_head, head_idx=h, cache=clean_cache)
                        model.add_hook(utils.get_act_name('z', l), freeze_hook_fn)
                        #fwd_hooks.append((utils.get_act_name('z', l), freeze_hook_fn))


            
            # model.run_with_hooks(
            #     orig_dataset.toks, fwd_hooks=fwd_hooks, reset_hooks_end=False
            # )

            _, patched_cache = model.run_with_cache(orig_dataset.toks, names_filter = lambda name: name.endswith('resid_post'))

            final_value = patched_cache['resid_post',-1]
            output = model.unembed(model.ln_final(final_value))
            result[layer, head] = patching_metric(output)
...
def patch_head_input(
    head_vector: Float[Tensor, 'batch post head_idx d_head'],
    hook: HookPoint,
    head_idx: int,
    cache: ActivationCache
):
...
def get_path_patch_head_to_heads(
    ...
    for layer in tqdm(range(model.cfg.n_layers)):
        for head in range(model.cfg.n_heads):  
            ...
            #     orig_dataset.toks, fwd_hooks=fwd_hooks, reset_hooks_end=False
            # )
            _, patched_cache = model.run_with_cache(orig_dataset.toks, names_filter = lambda name: name.endswith(receiver_input))

            fwd_hooks = []
            for h in receiver_heads:
                print(h)
                hook_fn = partial(patch_head_input, head_idx=h[1], cache=patched_cache)
                fwd_hooks.append((utils.get_act_name(receiver_input, h[0]), hook_fn))
                #model.add_hook(utils.get_act_name(receiver_input, head[0]), hook_fn)
