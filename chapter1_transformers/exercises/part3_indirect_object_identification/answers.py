...
from typing import List, Optional, Callable, Tuple, Dict, Literal, Set, Any
...
def logits_to_ave_logit_diff(
    ...
    '''
    n_batches, seq_len, d_vocab = logits.shape
    result = logits.new_zeros((n_batches,))
    for batch_idx in range(n_batches):
        answer_logits = logits[batch_idx, -1, answer_tokens[batch_idx, :]]
        result[batch_idx] = answer_logits[0] - answer_logits[1]

    result2 = t.gather(input=logits[:, -1, :], index=answer_tokens[:], dim=1)
    result2 = result2[:, 0] - result2[:, 1]
    result3 = logits[t.arange(n_batches)[:, None], -1, answer_tokens]
...
def residual_stack_to_logit_diff(
    ...
    avg_logit_diff = (scaled_residual_stack * logit_diff_directions).sum(-1).mean(-1)
...
def ioi_metric(
    ...
    '''
    n_batches, _, _ = logits.shape
    relevant_logits = logits[t.arange(n_batches)[:, None], -1, answer_tokens]
    observed_logit_diff = relevant_logits[:, 0] - relevant_logits[:, 1]
    return (observed_logit_diff - corrupted_logit_diff).divide(clean_logit_diff - corrupted_logit_diff).mean(dim=0)
...
def patch_residual_component(
    ...
    '''
    clean_residual = clean_cache[hook.name]
    corrupted_residual_component[:, pos] = clean_residual[:, pos]
...
def get_act_patch_resid_pre(
    ...
    results = corrupted_tokens.new_zeros(n_layers, seq_len).float()
    for layer in range(n_layers):
        for pos in range(seq_len):
            patch_res_hook = partial(patch_residual_component, pos=pos, clean_cache=clean_cache)
            patched_logits = model.run_with_hooks(corrupted_tokens, fwd_hooks=[(
                utils.get_act_name('resid_pre', layer), patch_res_hook
            )])
...
def patch_head_vector(
    ...
    '''
    clean_head_act = clean_cache[hook.name]
    corrupted_head_vector[:, :, head_index] = clean_head_act[:, :, head_index]
...
def get_act_patch_attn_head_out_all_pos(
    ...
    results = corrupted_tokens.new_zeros(n_layers, n_heads).float()
    for layer in range(n_layers):
        for head in range(n_heads):
            patch_head_hook = partial(patch_head_vector, head_index=head, clean_cache=clean_cache)
            patched_logits = model.run_with_hooks(corrupted_tokens, fwd_hooks=[(
                utils.get_act_name('z', layer), patch_head_hook
            )])
...
k = 4
top_heads = topk_of_Nd_tensor(act_patch_attn_head_all_pos_every_own[3], k=k)
...
# %%
a = patching.get_act_patch_attn_head_by_pos_every(model, corrupted_tokens, clean_cache, ioi_metric)
# %%
patching_result = a
imshow(patching_result, animation_frame=1, facet_col=0,
       facet_labels=["Output", "Query", "Key", "Value", "Pattern"],
       y=labels)

# %%
central_heads = [(3, 0), (5, 5), (6, 9), (7, 3), (7, 9), (8, 6), (8, 10), (9, 9), (10, 7), (11, 10)]
attn_patterns_central_heads = t.stack([cache['pattern', layer][:, head] for layer, head in central_heads], dim=1)

example = 1

display(cv.attention.attention_patterns(
    attention = attn_patterns_central_heads[example],
    tokens = model.to_str_tokens(tokens[example]),
    attention_head_names = [f"{layer}.{head}" for layer, head in central_heads],
))
# %%
...
def patch_path(
    ...
    model.reset_hooks()
    model.add_hook(
        utils.get_act_name("z", layer_idx),
        partial(patch_head_vector, head_index=head_idx, clean_cache=new_cache),
    )

    for layer_idx in range(layer_idx+1, model.cfg.n_layers):
        for head_idx in range(model.cfg.n_heads):
            model.add_hook(
                utils.get_act_name("z", layer_idx),
                partial(patch_head_vector, head_index=head_idx, clean_cache=orig_cache),
            )

    logits = model(orig_dataset.toks)
    model.reset_hooks()

    return metric(logits=logits)
...
def get_path_patch_head_to_final_resid_post(
    ...
    for layer_idx in range(model.cfg.n_layers):
        for head_idx in range(model.cfg.n_heads):
            result[layer_idx, head_idx] = patch_path(
                model=model,
                layer_idx=layer_idx,
                head_idx=head_idx,
                new_cache=new_cache,
