...
def logits_to_ave_logit_diff(
    ...
    answer_logits = t.gather(input=final_logits, dim=-1, index=answer_tokens)
    corr_logits, incorr_logits = answer_logits.unbind(dim=-1)
    logit_diffs = corr_logits - incorr_logits
    return logit_diffs if per_prompt else logit_diffs.mean()
...
def residual_stack_to_logit_diff(
    ...
    '''
    scaled_stack = cache.apply_ln_to_stack(residual_stack, layer=-1, pos_slice=-1)
    batch_size = residual_stack.size(-2)
    logit_diff_directions = einops.einsum(scaled_stack, logit_diff_directions, 
                                          '... batch d_model, batch d_model -> ...') 
    return logit_diff_directions / batch_size
...
def patch_residual_component(
    ...
    # patch in clean value to corrupted residual component
...
def get_path_patch_head_to_final_resid_post(
    ...
    return results
# %%
def patch_or_freeze_head_vectors(hook: HookPoint, 
