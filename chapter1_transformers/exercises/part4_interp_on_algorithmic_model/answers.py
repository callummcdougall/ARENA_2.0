...
def is_balanced_vectorized(tokens: Float[Tensor, "seq_len"]) -> bool:
    ...
    '''
    change_table = t.tensor([0,0,0,1,-1])
    changed_tokens = change_table[tokens]
    cum_sum = changed_tokens.cumsum(dim=-1)

    return cum_sum.min() >= 0 and cum_sum[-1] == 0
...
def get_ln_fit(
    ...
    '''
    resid_post_hook, final_ln_output_hook = LN_hook_names(layernorm)

    cache = get_activations(model, data.toks, [resid_post_hook, final_ln_output_hook])

    if seq_pos == None:
        # Aggregate across sequence and batch dimensions
        resid_activation = einops.rearrange(
            cache[resid_post_hook],
            "batch seq_len d_model -> (batch seq_len) d_model"
        ).cpu()
        ln_activation = einops.rearrange(
            cache[final_ln_output_hook],
            "batch seq_len d_model -> (batch seq_len) d_model"
        ).cpu()
    else:
        resid_activation = cache[resid_post_hook][:, seq_pos, :].cpu()
        ln_activation = cache[final_ln_output_hook][:, seq_pos, :].cpu()
    reg = LinearRegression()

    reg.fit(resid_activation, ln_activation)

    return (reg, reg.score(resid_activation, ln_activation))
...
def get_pre_final_ln_dir(model: HookedTransformer, data: BracketsDataset) -> Float[Tensor, "d_model"]:
    ...
    L_final = t.tensor(
        get_ln_fit(model, data, model.ln_final, 0)[0].coef_
    ).T.to(device)

    return L_final @ get_post_final_ln_dir(model).to(device)
...
def get_out_by_components(model: HookedTransformer, data: BracketsDataset) -> Float[Tensor, "component batch seq_pos emb"]:
    ...
    hook_names = ['hook_embed', 'hook_pos_embed'] + \
                [utils.get_act_name('result', i) for i in range(model.cfg.n_layers)] + \
                [utils.get_act_name('mlp_out', i) for i in range(model.cfg.n_layers)]
    
    cache = get_activations(model, data.toks, hook_names)
    # Reshape layer 0 heads
    reshaped_heads = [
        t.concat(
            [
                einops.rearrange(
                    cache['result', i],
                    "dataset_size seq_pos head_num emb -> head_num dataset_size seq_pos emb"
                )
            ]
        ) for i in range(model.cfg.n_layers)
    ]

    reshaped_mlp = [t.unsqueeze(cache['mlp_out', i], dim=0) for i in range(model.cfg.n_layers)]
    reshaped_emb = (cache["hook_embed"] + cache["hook_pos_embed"]).unsqueeze(0)

    out_by_components = t.concat(
        [
            reshaped_emb,
            reshaped_heads[0], reshaped_mlp[0], 
            reshaped_heads[1], reshaped_mlp[1],
            reshaped_heads[2], reshaped_mlp[2],
        ], dim=0
    )
...
if MAIN:
    ...
    balanced_batches = out_by_component_in_unbalanced_dir[:, data.isbal]
    out_by_component_in_unbalanced_dir -= balanced_batches.mean(dim=1).unsqueeze(1)
...
def is_balanced_vectorized_return_both(
    ...
    change_table = t.tensor([0,0,0,1,-1]).to(device)
    negative_failure = change_table[toks].flip(-1).cumsum(dim=-1).max(dim=-1).values > 0 
    total_elevation_failure = change_table[toks].cumsum(dim=-1)[:, -1] != 0
...
def get_WOV(model: HookedTransformer, layer: int, head: int) -> Float[Tensor, "d_model d_model"]:
    ...
    W_OV = einops.einsum(
        ...
        "d_model_o seq_len, seq_len d_model_v -> d_model_o d_model_v"
    )
    return W_OV
...
def get_pre_20_dir(model, data) -> Float[Tensor, "d_model"]:
    ...
    layer_norm_1 = t.tensor(
        get_ln_fit(model, data, model.blocks[2].ln1, 1)[0].coef_
    ).T.to(device)

    W_OV = get_WOV(model, 2, 0)

    intermediate = einops.einsum(
        layer_norm_1,
        W_OV,
        "d_model_l d_model, d_model d_model_v -> d_model_l d_model_v"
    )

    pre_20_dir = einops.einsum(
        intermediate,
        pre_final_ln_dir,
        "d_model d_model_i, d_model_i -> d_model"
    )
...
if MAIN:
    ...
    out_by_component = get_out_by_components(model, data)
    unbalanced_dir = get_pre_20_dir(model, data)
    
    out_by_component_in_pre_20_unbalanced_dir = einops.einsum(
        out_by_components[:7, :, 1, :],
        unbalanced_dir,
        "component batch d_model, d_model -> component batch"
    )
    balanced_batches = out_by_component_in_pre_20_unbalanced_dir[:, data.isbal]
    out_by_component_in_pre_20_unbalanced_dir -= balanced_batches.mean(dim=1).unsqueeze(1)
...
def get_out_by_neuron(
    ...
    post_mlp = get_activations(model, data.toks, [hook_name])[hook_name]
    if seq is None:
        out = einops.einsum(
            post_mlp,
            model.W_out[layer, ...],
            "batch seq_len neuron, neuron d_model -> batch seq_len neuron d_model"
        )
    else:
        out = einops.einsum(
            post_mlp[:, seq, ...],
            model.W_out[layer, ...],
            "batch neuron, neuron d_model -> batch neuron d_model"
        )
    return out
...
def get_out_by_neuron_in_20_dir(model: HookedTransformer, data: BracketsDataset, layer: int) -> Float[Tensor, "batch neurons"]:
    ...
    '''
    out_by_neuron = get_out_by_neuron(model, data, layer, seq=1)
    unbalanced_dir = get_pre_20_dir(model, data)

    get_out_by_neuron_in_20_dir = einops.einsum(
        out_by_neuron,
        unbalanced_dir,
        "batch neuron d_model, d_model -> batch neuron"
    )

    return get_out_by_neuron_in_20_dir
...
def get_out_by_neuron_in_20_dir_less_memory(model: HookedTransformer, data: BracketsDataset, layer: int) -> Float[Tensor, "batch neurons"]:
    ...
    post_mlp = get_activations(model, data.toks, [hook_name])[hook_name]
    unbalanced_dir = get_pre_20_dir(model, data)
    output_in_unbalanced_dir = einops.einsum(
        model.W_out[layer, ...],
        unbalanced_dir,
        "d_model_out d_model, d_model -> d_model_out"
    )

    out = einops.einsum(
        post_mlp[:, 1, ...],
        output_in_unbalanced_dir,
        "batch neuron, neuron -> batch neuron"
    )
...
def get_q_and_k_for_given_input(
    ...
    '''
    hook_name = [utils.get_act_name('q', layer), utils.get_act_name('k', layer)]
    ...
    return cache[hook_name[0]].squeeze(), cache[hook_name[1]].squeeze()
...
L_embed = embedding(model, tokenizer, "(").T
R_embed = embedding(model, tokenizer, ")").T

layer_norm_1 = t.tensor(
            get_ln_fit(model, data, model.blocks[0].ln1, 1)[0].coef_
        ).T.to(device)

W_OV = get_WOV(model, 0, 0)

v_L_inter = einops.einsum(
    L_embed,
    layer_norm_1,
    "d_model, d_model d_model_n -> d_model_n"
)

v_L = einops.einsum(
    v_L_inter,
    W_OV,
    "d_model, d_model d_model_w -> d_model_w"
)

v_R_inter = einops.einsum(
    R_embed,
    layer_norm_1,
    "d_model, d_model d_model_n -> d_model_n"
)
...
def avg_squared_cos_sim(v: Float[Tensor, "d_model"], n_samples: int = 1000) -> float:
    ...
    '''
    rand_vec = t.rand(n_samples, v.shape[0]).to(device)

    return t.cosine_similarity(rand_vec, v, dim=-1).mean()
# %%
