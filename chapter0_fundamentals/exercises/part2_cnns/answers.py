...
if MAIN:
    ...
    display_array_as_img(
        einops.reduce(arr.astype(float), 'nums c h w -> h w', 'min')
    )
...
def conv2d_minimal(x: Float[Tensor, "b ic h w"], weights: Float[Tensor, "oc ic kh kw"]) -> Float[Tensor, "b oc oh ow"]:
    ...
    strided_x = x.as_strided(new_shape, new_stride)
    return einops.einsum(strided_x, weights, 'b ic kh oh kw ow, oc ic kh kw -> b oc oh ow')
...
class Flatten(nn.Module):
    ...
    def forward(self, x: t.Tensor) -> t.Tensor:
        ...
        '''
        out = x.reshape(x.shape[:self.start_dim]+(-1,)+x.shape[(self.end_dim % len(x.shape))+1:])
        return out
...
class Conv2d(nn.Module):
    def __init__(
        ...
        self.weight = nn.Parameter(torch.zeros(out_channels, in_channels, *force_pair(kernel_size)))
    ...
    def extra_repr(self) -> str:
        return ', '.join(f'{name}={getattr(self, name)}' for name in
