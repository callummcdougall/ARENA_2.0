...
class Flatten(nn.Module):
    ...
    def forward(self, input: t.Tensor) -> t.Tensor:
        ...
        left = shape[:start_dim]
        right = shape[end_dim+1:]
        mid = functools.reduce(lambda x, y: x*y, shape[start_dim : end_dim+1])

        return input.reshape(left + (mid,) + right)
...
class Linear(nn.Module):
    def __init__(self, in_features: int, out_features: int, bias=True):
        ...
        '''
        super().__init__()
        normalize = lambda x: (x - 0.5) * 2 / t.tensor(in_features).sqrt()
        self.weight = nn.Parameter(normalize(t.rand(out_features, in_features)))
        self.bias = nn.Parameter(normalize(t.rand(out_features))) if bias else None
...
class Conv2d(nn.Module):
    def __init__(
        ...
        normalize = lambda x: (x - 0.5) * 2 / t.tensor(k).sqrt()
        self.weight = nn.Parameter(
            normalize(
                t.rand(out_channels, in_channels, kernel_height, kernel_width)
            )
        )
        self.stride = stride
        self.padding = padding
