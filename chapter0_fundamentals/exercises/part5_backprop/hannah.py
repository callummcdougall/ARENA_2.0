...
class BackwardFuncLookup:
    def __init__(self) -> None:
        ...
        # default value will be an empty dictionary
        # so don't need to manually initialise nested dictionaries for each key
...
def log_forward(x: Tensor) -> Tensor:
    ...
    out = Tensor(np.log(x_array), requires_grad)
    if requires_grad:
        out.recipe = Recipe(
            func = np.log,
            args = (x_array,),
...
def multiply_forward(a: Union[Tensor, int], b: Union[Tensor, int]) -> Tensor:
    ...
    requires_grad = grad_tracking_enabled and any(x.requires_grad for x in [a,b] if isinstance(x, Tensor))
    # if ANY of the inputs require grad, we need to calculate the grad on the output

    out = Tensor(func(*args), requires_grad)
...
def wrap_forward_fn(numpy_func: Callable, is_differentiable=True) -> Callable:
    ...
    def tensor_func(*args: Any, **kwargs: Any) -> Tensor:
        # the args are tensors, and we want to be able to pass them to the function and return a tensor

        arrays = tuple([arg.array if isinstance(arg, Tensor) else arg for arg in args])

        # for teh args which are tensors, check if they require grad OR have a recipe

        requires_grad = grad_tracking_enabled and is_differentiable and any([(arg.requires_grad or arg.recipe is not None) for arg in args if isinstance(arg, Tensor)])

        

        out = Tensor(numpy_func(*arrays, **kwargs), requires_grad)
...
def topological_sort(node: Node, get_children: Callable) -> List[Node]:
    ...
    '''
    out = []
    visiting_children = set()

    def visit(node):
        # backprop weirdly breaks if we just write 'if node in out'
        if node in set(out):
            return
        if node in visiting_children: #if you visit a node whose children are being visited!
            raise ValueError('contains cycle!')
        visiting_children.add(node)
        for child in get_children(node):
            visit(child)

        visiting_children.remove(node)
        out.append(node)
...
def backprop(end_node: Tensor, end_grad: Optional[Tensor] = None) -> None:
    ...
    for node in nodes:
        if node.is_leaf and node.requires_grad:
            ...
            else: #node.grad might already have a value
                node.grad += grad[node]

        #if node has a recipe (requires_grad => has recipe)
        elif node.recipe and node.recipe.parents: #need to edit if statement
            fn = node.recipe.func
            grad_out = grads[node]
            out = node.array
            ...
            for i, parent in node.recipe.parents.items():
                # parents are already tensors
                # get the ith back function, and wrap
                back_fn = wrap_forward_fn(BACK_FUNCS.get_back_func(fn, i))
                #unsure if wrapping is the right thing to do here
                grad = back_fn(grad_out, out, *args, **kwargs)
                if parent not in grads.keys():
                    grads[parent] = grad
                else:
                    grads[parent] += grad
...
def invert_transposition(axes: tuple) -> tuple:
    ...
    '''
    pairs = sorted(list(enumerate(axes)), key=lambda x: x[1])

    out, b = list(zip(*pairs))
    return out
...
class Module:
    ...
    def parameters(self, recurse: bool = True) -> Iterator[Parameter]:
        ...
        '''
        out = list(self.__dict__['_parameters'].values())
        if recurse:
            for module in self.modules():
                out.extend(list(module.parameters(recurse)))
        return iter(out)
...
class Linear(Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias=True):
        ...
        k = 1 / in_features
        self.weight = Parameter(Tensor(np.random.rand(in_features, out_features) - 0.5) * 2 * (k ** 0.5))
        self.bias = Parameter(Tensor(np.random.rand(out_features) - 0.5) * 2 * (k ** 0.5)) if bias else None
...
def cross_entropy(logits: Tensor, true_labels: Tensor) -> Tensor:
    ...
    logits_true = logits[arange(0,B), true_labels]
    log_probs = logits_true - log(sum(exp(logits), dim=-1))
