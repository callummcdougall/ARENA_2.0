...
def backprop(end_node: Tensor, end_grad: Optional[Tensor] = None) -> None:
    ...
    if end_grad is None:
        end_grad = Tensor(np.ones_like(end_node.array), requires_grad=False)
    ...
    for node in sorted_computational_graph(end_node):
        ...
        if node.is_leaf and node.requires_grad:
            if node.grad is None:
                node.grad = Tensor(grad_out, requires_grad=False)
            else:
                node.grad += Tensor(grad_out, requires_grad=False)
        ...
        for argnum, parent in node.recipe.parents.items():
            ...
            back_func = BACK_FUNCS.get_back_func(fwd_func, argnum)

            # Use this backward function to calculate the gradient
            grad = back_func(grad_out, node.array, *args, **kwargs)
...
class Module:
    ...
    def parameters(self, recurse: bool = True) -> Iterator[Parameter]:
        ...
        '''
        # This is technically bad to double yield, but whatever
        parameters = self.__dict__["_parameters"].values()
        for m in parameters:
            yield m
            if recurse:
                for n in m.parameters(True):
                    yield n
...
class Linear(Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias=True):
        ...
        w = sf * Tensor(2 * (np.random.rand(out_features, in_features) - 1))
        b = sf * Tensor(2 * (np.random.rand(out_features) - 1))
        if bias:
            self.bias = Parameter(b)
        self.weight = Parameter(w, requires_grad=True)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.bias is not None:
            return self.weight @ x + self.bias.T
        else:
