...
def log_forward(x: Tensor) -> Tensor:
    ...
    out_req_grad = grad_tracking_enabled and (x.requires_grad or x.recipe is not None)
    out_tensor = Tensor(out_arr, out_req_grad)
    if out_req_grad:
        recipe = Recipe(func=np.log, args=(x.array,), kwargs=dict(), parents={0: x})
...
def multiply_forward(a: Union[Tensor, int], b: Union[Tensor, int]) -> Tensor:
    ...
    out_tensor = Tensor(np.multiply(a_arr, b_arr), requires_grad)
    if parents:
        out_tensor.recipe = Recipe(func=np.multiply, args=(a_arr, b_arr), 
...
def wrap_forward_fn(numpy_func: Callable, is_differentiable=True) -> Callable:
    ...
    def tensor_func(*args: Any, **kwargs: Any) -> Tensor:
        parents = {idx: x for idx, x in enumerate(args) if isinstance(x, Tensor)}
        args_array = tuple((x.array if isinstance(x, Tensor) else x) for x in args)
        requires_grad = is_differentiable and any([x.requires_grad for x in parents.values()])
        
        out_tensor = Tensor(numpy_func(*args_array, **kwargs), requires_grad)
        if requires_grad:
            out_tensor.recipe = Recipe(numpy_func, args_array, kwargs, parents)
...
def sorted_computational_graph(tensor: Tensor) -> List[Tensor]:
    ...
    '''
    def get_recipe_parents(tensor: Tensor) -> List[Tensor]:
        if tensor.recipe is None:
            return []
        return tensor.recipe.parents.values()
    
    return reversed(topological_sort(tensor, get_recipe_parents))
...
def backprop(end_node: Tensor, end_grad: Optional[Tensor] = None) -> None:
    ...
    # if end_grad is None:
    #     end_grad = np.array([1.])
    # if not isinstance(end_grad, Tensor):
    #     end_grad = Tensor(end_grad, requires_grad=True)

    # if end_node.requires_grad:
    #     if end_node.is_leaf:
    #         if end_node.grad is None:
    #             end_node.grad = Tensor(np.zeros_like(end_node.array, dtype='float64'), requires_grad=False)
    #         end_node.grad.array += end_grad.array # for accumulation
        
    #     else:
    #         recipe_func = end_node.recipe.func
            
    #         for idx,parent_node in end_node.recipe.parents.items():
    #             # the callable function that gives the gradient for parent node
    #             node_back_func = BACK_FUNCS.get_back_func(recipe_func, idx)
    #             parent_end_grad = node_back_func(end_grad.array, end_node.array, \
    #                         *end_node.recipe.args, **end_node.recipe.kwargs)
    #             backprop(parent_node, parent_end_grad)

    
    sorted_nodes = sorted_computational_graph(end_node)

    if end_grad is None:
        end_grad = np.ones_like(end_node, dtype='float64')
    if isinstance(end_grad, Tensor):
        end_grad = end_grad.array
    gradient_values: Dict[Tensor, Arr] = {}
    gradient_values[end_node] = end_grad

    for node in sorted_nodes:
        if node.requires_grad:
            if node.is_leaf:
                node.grad = Tensor(gradient_values[node], requires_grad=False)

            else:
                recipe_func = node.recipe.func
                for idx,parent_node in node.recipe.parents.items():
                    # the callable function that gives the gradient for parent node
                    node_back_func = BACK_FUNCS.get_back_func(recipe_func, idx)
                    parent_end_grad = node_back_func(gradient_values[node], node.array, \
                                *node.recipe.args, **node.recipe.kwargs)
                    
                    if parent_node not in gradient_values:
                        gradient_values[parent_node] = np.zeros_like(parent_node.array, dtype='float64')
                    gradient_values[parent_node] += parent_end_grad
...
def exp_back(grad_out: Arr, out: Arr, x: Arr) -> Arr:
    return unbroadcast(grad_out * out, out)
...
def invert_transposition(axes: tuple) -> tuple:
    ...
    '''
    permute_dict = {ax: i for i, ax in enumerate(axes)}
    return tuple((permute_dict[i] for i in range(len(axes))))
