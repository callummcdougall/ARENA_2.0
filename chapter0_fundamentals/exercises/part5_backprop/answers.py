...
class BackwardFuncLookup:
    def __init__(self) -> None:
        self.funcs: Dict[Tuple[Callable, int], Callable] = {}
...
def multiply_forward(a: Union[Tensor, int], b: Union[Tensor, int]) -> Tensor:
    ...
    val_a = a.array if isinstance(a, Tensor) else a
    val_b = b.array if isinstance(b, Tensor) else b
    
    out_val = np.multiply(val_a, val_b)
...
def wrap_forward_fn(numpy_func: Callable, is_differentiable=True) -> Callable:
    ...
    def tensor_func(*args: Any, **kwargs: Any) -> Tensor:
        assert any(isinstance(val, Tensor) for val in args)

        vals = [val.array if isinstance(val, Tensor) else val for val in args]
        
        out_val = numpy_func(*vals, **kwargs)

        requires_grad = grad_tracking_enabled \
            and any((isinstance(x, Tensor) and (x.requires_grad or x.recipe is not None)) for x in args) \
...
def sorted_computational_graph(tensor: Tensor) -> List[Tensor]:
    ...
    '''
    def get_children(tensor) -> List[Tensor]:
...
def backprop(end_node: Tensor, end_grad: Optional[Tensor] = None) -> None:
    ...
    '''
    end_grad = Tensor(np.ones_like(end_node.array)) if end_grad is None else end_grad

    graph = sorted_computational_graph(end_node)

    gradients: Dict[Tensor, Tensor] = {end_node: end_grad}

    for node in graph:
        if node.recipe is None:
            continue

        for parent_ix, parent in node.recipe.parents.items():
            back_func = BACK_FUNCS.get_back_func(node.recipe.func, parent_ix)
            grad = back_func(gradients[node].array, node.array, *node.recipe.args, **node.recipe.kwargs)
            if parent not in gradients:
                gradients[parent] = Tensor(grad)
            else:
                gradients[parent].array += grad

    # END: set gradients for nodes
    for node in graph:
        if node.is_leaf and node.requires_grad:
            if node.grad is None:
                node.grad = gradients[node]
            else:
                node.grad.array += gradients[node].array
