...
def opt_fn_with_sgd(fn: Callable, xy: t.Tensor, lr=0.001, momentum=0.98, n_iters: int = 100):
    ...
    '''
    optimizer = optim.SGD([xy], lr, momentum)
    values = []
    for _ in range(n_iters):
        values.append(xy.tolist())
        loss = fn(*xy)
        loss.backward()
        optimizer.step()
        optimizer.zero_grad()
    return t.tensor(values)
# %%
if MAIN:
    points = []
...
class SGD:
    def __init__(
        ...
        self.running_grads = [t.zeros_like(param) for param in self.params]
        self.lr = lr
    ...
    @t.inference_mode()
    def step(self) -> None:
        for param, running_grad in zip(self.params, self.running_grads):
            g = param.grad + self.weight_decay * param
            g = g + self.momentum * running_grad
            param -= self.lr * g
            running_grad.set_(g)
...
class RMSpropOurs:
    ...
    @t.inference_mode()
    def step(self) -> None:
        for param, running, running_squared in zip(self.params, self.running_grad,
                                                   self.running_squared_grad):
            g = param.grad + self.weight_decay * param
            running_squared.set_(self.alpha * running_squared +
                                 (1-self.alpha) * g.pow(2))
            if self.momentum > 0:
                running.set_(self.momentum * running +
                             g/(running_squared.sqrt() + self.eps))
                param.subtract_(self.lr * running)
            else:
                param.__isub__(self.lr * g / (running_squared.sqrt()
                                                    + self.eps))

    def __repr__(self) -> str:
        return f"RMSprop(lr={self.lr}, eps={self.eps}, momentum={self.momentum}, weight_decay={self.weight_decay}, alpha={self.alpha})"

#! From solutions - ours is different
class RMSprop:
    def __init__(
        self,
        params: Iterable[t.nn.parameter.Parameter],
...
if MAIN:
    tests.test_rmsprop(RMSpropOurs)
...
class AdamOurs:
    ...
    def step(self) -> None:
        ...
        for param, running, running_squared in zip(self.params, self.running_grad,
                                                   self.running_squared_grad):
            g = param.grad + self.weight_decay * param
            running.set_(self.beta1 * running + (1-self.beta1) * g)
            running_squared.set_(self.beta2 * running_squared + (1-self.beta2) * g.pow(2))
            moment_bias_corrected = running / (1-self.beta1**self.t)
            moment2_bias_corrected = running_squared / (1-self.beta2**self.t)
            param.subtract_(self.lr * moment_bias_corrected
                            / (moment2_bias_corrected.sqrt() + self.eps))
...
if MAIN:
    ...
    for optimizer_class, params in optimizer_list:
        xy = t.tensor([2.5, 2.5], requires_grad=True)
        xys = opt_fn(rosenbrocks_banana_func, xy=xy, optimizer_class=optimizer_class, optimizer_hyperparams=params, n_iters=400)
        points.append((xys, optimizer_class, params))
...
def train():
    args = ResNetFinetuningArgsWandb(trainset=cifar_trainset_small,
                                     testset=cifar_testset_small)
    for k in ["learning_rate", "batch_size", "max_epochs"]:
    ...
    trainer.fit(model=model, train_dataloaders=args.trainloader,
                val_dataloaders=args.testloader)

# %%
if MAIN:
    sweep_id = wandb.sweep(sweep=sweep_config, project='day4-resnet-sweep')
    wandb.agent(sweep_id=sweep_id, function=train, count=32)
