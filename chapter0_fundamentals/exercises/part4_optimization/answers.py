...
def opt_fn_with_sgd(fn: Callable, xy: t.Tensor, lr=0.001, momentum=0.98, n_iters: int = 100):
    ...
    '''
    # check that requires_grad = true for the xy tensor
    assert xy.requires_grad

    # create tensor that will eventually contain all (x, y) pairs
    xys = t.zeros((n_iters, 2))

    # define our optimizer as SGD on the xy tensor 
    # with the learning rate and momentum given as inputs 
    optimizer = optim.SGD([xy], lr=lr, momentum=momentum)

    # run optimization for n_iters iterations
    for i in range(n_iters):
        # add current (x, y) to xys tensor
        xys[i] = xy.detach()
        # define output of function
        out = fn(xy[0], xy[1])
        # execute backprop to find gradient
        out.backward()
        # execute one gradient descent step
        optimizer.step()
        # zero gradients in preperation for next loop iteration
        optimizer.zero_grad()
...
class SGD:
    def __init__(
        ...
        # define learning rate, momentum, weight decay, and time step
        self.lr = lr
        self.mu = momentum
        self.lmda = weight_decay
        self.t = 0

        # initialize gradients for all params
...
class RMSprop:
    def __init__(
        ...
        # define learning rate, alpha, eps, momentum, weight decay, and time step
        self.lr = lr
        self.alpha = alpha
        self.eps = eps
        self.mu = momentum
    ...
    @t.inference_mode()
    def step(self) -> None:
        for i, (param, b, v) in enumerate(zip(self.params, self.bs, self.vs)):
            # Implement the algorithm from the pseudocode to get new values of params and g
            new_g = param.grad
            if self.lmda != 0:
                new_g = new_g + (self.lmda * param)
            ...
            if self.mu > 0:
                ...
                param -= self.lr * new_g / (new_v.sqrt() + self.eps)
