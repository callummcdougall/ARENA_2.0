...
if MAIN:
    ...
    optimizer_list += [(optim.SGD, {"lr": 0.4, "momentum": 0.9 + x/1000}) for x in range(0, 101, 10)]
    n_iters = 500

    for optimizer_class, params in optimizer_list:
        xy = t.tensor([2.5, 2.5], requires_grad=True)
        xys = opt_fn_with_sgd(pathological_curve_loss, xy=xy, lr=params['lr'], momentum=params['momentum'], n_iters=n_iters)
