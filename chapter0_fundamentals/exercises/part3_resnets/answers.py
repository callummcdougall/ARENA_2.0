...
class ConvNet(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = Conv2d(
            in_channels=1, out_channels=32, kernel_size=(3, 3), padding=1, stride=1
        )  # 32, 28, 28
        self.relu = ReLU()
...
class ConvNetTrainingArgs:
    ...
    def __post_init__(self):
        ...
        self.trainloader = DataLoader(
            trainset, shuffle=True, batch_size=self.batch_size
        )
...
if MAIN:
    ...
    trainer = pl.Trainer(
        max_epochs=args.max_epochs, logger=args.logger, log_every_n_steps=1
    )
