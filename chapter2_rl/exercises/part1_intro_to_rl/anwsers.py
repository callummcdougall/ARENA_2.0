...
def policy_eval_numerical(env: Environment, pi: Arr, gamma=0.99, eps=1e-8, max_iterations=10_000) -> Arr:
    ...
    count = 0
    delta = 100
    while delta >= eps and count<max_iterations:
        delta = 0
        for state in range(num_states):
            v = V[state]
            trans_probs = env.T[state, pi[state], :] # size (num_states)
            rewards = env.R[state, pi[state], :]
            V[state] = np.sum(
                trans_probs * (rewards + gamma * V )
            )
            delta = np.max(( delta, np.abs( v - V[state])))
        count += 1
...
def policy_improvement(env: Environment, V: Arr, gamma=0.99) -> Arr:
    ...
    '''
    num_states = len(V)
    better_pi = np.empty(num_states, dtype= int)
    for state in range(num_states):
        summand = env.T[state] * (env.R[state] + gamma * np.expand_dims(V, 0))
        action_values = np.sum(summand, axis=-1)
        better_pi[state] = np.argmax(action_values)
    return better_pi
