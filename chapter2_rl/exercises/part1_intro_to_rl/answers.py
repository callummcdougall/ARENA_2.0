...
class RewardAveraging(Agent):
    ...
    def get_action(self):
        if self.rng.random() < self.epsilon:
            ...
            return np.argmax(self.arm_reward_means)

    def observe(self, action, reward, info):
        self.arm_counts[action] += 1
        self.arm_reward_means[action] += (reward - self.arm_reward_means[action]) / self.arm_counts[action]

    def reset(self, seed: int):
        super().reset(seed)
        self.arm_reward_means = np.full(shape=[self.num_arms], dtype=float, fill_value=optimism)
        self.arm_counts = np.zeros(shape=[self.num_arms], dtype=int)
...
# %%
'''
Value of policy at given state =
    Average likelihood of taking an action in a policy given the state * 
    Average reward gotten at the next state

Where the average reward gotten at the next state = 
    Average likelihood of moving onto the the next state given this state s and action a * 
    Average reward at the next state + discounted value of the next state


Computing the value V_pi (s_0) for pi = pi_L and pi = pi_R

These policies are equal when discount_factor = 1/2
...
def policy_eval_numerical(env: Environment, pi: Arr, gamma=0.99, eps=1e-8, max_iterations=10_000) -> Arr:
    ...
    for i in range(max_iterations):
        ...
        for state in range(env.num_states):
            action = pi[state]
            next_state, reward, prob = env.dynamics(state, action)
            next_rewards = np.array([prev_values[s] for s in next_state])
            expected_reward = np.dot(prob, next_rewards + reward)
            values[state] = gamma * expected_reward
            prev_values = values
