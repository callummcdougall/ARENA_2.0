...
class RewardAveraging(Agent):
    ...
    def observe(self, action, reward, info):
        self.n[action] += 1
        self.reward_estimate[action] = self.reward_estimate[action] + 1/self.n[action] * (reward - self.reward_estimate[action])

    def reset(self, seed: int):
        self.reward_estimate = np.ones((num_arms,)) * self.optimism
...
class UCBActionSelection(Agent):
    ...
    def get_action(self):
        ucb = self.reward_estimate + self.c * (np.sqrt(np.log(self.t) / (self.eps + self.n)))
    ...
    def reset(self, seed: int):
        self.reward_estimate = np.zeros((num_arms,))
        self.n = np.zeros((num_arms,))
        self.rng = np.random.default_rng(seed)
        self.t = 1
...
def policy_eval_numerical(env: Environment, pi: Arr, gamma=0.99, eps=1e-8, max_iterations=10_000) -> Arr:
    ...
    '''
    value_estimates = np.zeros(env.num_states)
    for _ in range(max_iterations):
        next_states = env.T[np.arange(env.num_states), pi] # state, next_state
        rewards = env.R[np.arange(env.num_states), pi] # state, next_state
        value_estimates_new = (next_states * (rewards + gamma * value_estimates)).sum(axis=-1)
        if np.allclose(value_estimates, value_estimates_new, rtol=1e-10, atol=1e-10):
            return value_estimates
        else:
            value_estimates = value_estimates_new
...
def policy_eval_exact(env: Environment, pi: Arr, gamma=0.99) -> Arr:
    ...
    next_states = env.T[np.arange(env.num_states), pi] # state, next_state
    rewards = env.R[np.arange(env.num_states), pi] # state, next_state    
    r = np.diag(next_states @ rewards.T)
    return np.linalg.solve(np.eye(env.num_states) - gamma * next_states, r)
...
def policy_improvement(env: Environment, V: Arr, gamma=0.99) -> Arr:
    ...
    '''
    next_states = env.T # state, action, next_state
    rewards = env.R # state, action, next_state
    value_estimates_new = (next_states * (rewards + gamma * V)).sum(axis=-1)
    return value_estimates_new.argmax(axis=-1)
...
def find_optimal_policy(env: Environment, gamma=0.99, max_iterations=10_000):
    ...
    for i in range(max_iterations):
        pi_new = policy_improvement(env, policy_eval_exact(env, pi, gamma), gamma)
