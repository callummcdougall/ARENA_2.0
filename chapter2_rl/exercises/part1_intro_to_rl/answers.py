...
class RewardAveraging(Agent):
    ...
    def observe(self, action, reward, info):
        self.rewards[action] = (reward + self.rewards[action] * self.num_samples[action]) / (self.num_samples[action] + 1)
        self.num_samples[action] += 1

    def reset(self, seed: int):
        super().reset(seed)

        self.num_samples = np.array([0 for i in range(self.num_arms)])
        self.rewards = np.array([self.optimism for i in range(self.num_arms)], dtype=float)
...
def policy_eval_numerical(env: Environment, pi: Arr, gamma=0.99, eps=1e-8, max_iterations=10_000) -> Arr:
    ...
    '''
    T, R = env.T, env.R # num_states, num_actions, num_states

    num_states = T.shape[0]
    val = np.zeros(num_states)
    
    for i in range(max_iterations):
        new_val = np.zeros(num_states)
        for state_x, action in enumerate(pi):
            prob_per_state = T[state_x][action]
            reward_per_state = R[state_x][action]
            discounted_reward_per_state = gamma * val
            new_val[state_x] = np.dot(prob_per_state, reward_per_state + discounted_reward_per_state)
        if np.abs(new_val - val).sum() < eps:
            break
        else:
            val = np.copy(new_val)
