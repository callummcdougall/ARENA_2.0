...
class RewardAveraging(Agent):
    epsilon: float
    optimisim: float
    value: Float[Arr, "num_arms"]
    n_observed: Float[Arr, "num_arms"]
    ...
    def get_action(self):
        ...
        if self.epsilon < entropy:
            return self.value.argmax()
        else:
            return self.rng.integers(low=0, high=self.num_arms).item()

    def observe(self, action, reward, info):
        self.n_observed[action] += 1
        self.value[action] += (reward - self.value[action]) / self.n_observed[action]

    def reset(self, seed: int):
        super().reset(seed)
        self.value = self.optimism * np.ones((self.num_arms,))
        self.n_observed = np.zeros((self.num_arms,))
...
class UCBActionSelection(Agent):
    ...
    ɛ: float
    t: int
    Q: Float[Arr, "num_arms"]
    N: Float[Arr, "num_arms"]

    def __init__(self, num_arms: int, seed: int, c: float, ɛ: float = 1e-6):
        self.c = c
        self.ɛ = ɛ
        super().__init__(num_arms, seed)

    def get_action(self):
        return np.argmax(self.Q + self.c * np.sqrt(np.log(self.t) / self.N + self.ɛ))
...
# %%
def policy_eval_numerical(env: Environment, pi: Int[Arr, "num_states"], gamma=0.99, eps=1e-8, max_iterations=10_000) -> Float[Arr, "num_states"]:
    '''
    ...
    for i in range(max_iterations):
        ...
        for s in range(num_states):
            new_V[s] = T[s, pi[s]].dot(R[s, pi[s]] + gamma * V)
        delta = np.abs(new_V - V).sum()
        if delta < eps:
            break
        V = new_V
    else:
