...
def compute_advantages(
    ...
    '''
    T = values.shape[0]
    next_values = t.cat((values,next_value),dim=0)[1:]
    next_dones = t.cat((dones, next_done), dim=0)[1:]
    deltas = rewards + gae_lambda * next_values[1:] * (1 - next_dones) - values # (buffer_size, env)
    factors = (gamma * gae_lambda) ** t.arange(len(deltas))
    advantages = t.zeros_like(deltas)
    advantages[-1] = deltas[-1]

    for time in reversed(range(T-1)):
        advantages[time] = advantages[time + 1 ] * gamma * gae_lambda * (1 - next_dones[time + 1]) + deltas[time]
