...
prof.export_chrome_trace("trace_nograd_bigbatch_64.json")
...
def test_softmax_random_input(fn1, fn2):
    ...
    assert torch.allclose(fn1(x), fn2(x), 1e-5, 1e-10), "Implementations are not analogous"
    print('Tests passed')
...
def quantize_tensor(x, min_val=None, max_val=None, num_bits=8) -> QTensor:
    ...
    max_val = x.max() if not max_val else max_val
    min_val = x.min() if not min_val else min_val

    if (max_val - min_val) < 1e-7:
        return QTensor(torch.zeros_like(x, dtype=torch.uint8), 0, 0)

    scale = (max_val - min_val) / (qmax - qmin)
    prop = ((qmin - min_val) / (max_val - min_val))
    zero_point = int(qmax * prop)
    x_quant = ((x / scale) + zero_point).round_()
    return QTensor(x_quant.to(torch.uint8), scale, zero_point)

# test_quantize_tensor(quantize_tensor)
...
def quantizeLayer(x, layer, stat, scale_x, zp_x) -> Tuple[torch.tensor, float, float]:
    ...
    scale, zero_point = calcScaleZeroPoint(stat['min'], stat['max'])

    weight = layer.weight.data
    bias = layer.bias.data

    quantized_weight = quantize_tensor(layer.weight.data)
    quantized_bias = quantize_tensor(layer.bias.data)

    layer.weight.data = quantized_weight.tensor.float()
    layer.bias.data = quantized_bias.tensor.float()

    x_prepped = x.float() - zp_x

    layer.weight.data = (scale_x * quantized_weight.scale / scale) \
                      * (layer.weight.data - quantized_weight.zero_point)
    layer.bias.data = (quantized_bias.scale / scale) \
                    * (layer.bias.data + quantized_bias.zero_point)


    layer_out = layer(x_prepped)
    out = F.relu(layer_out)
