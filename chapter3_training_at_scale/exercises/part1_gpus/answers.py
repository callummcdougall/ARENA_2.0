...
def quantize_tensor(x, min_val=None, max_val=None, num_bits=8) -> QTensor:
    ...
    zero_point = torch.tensor(-min_val/scale).clamp(qmin, qmax).round().item()
    result = (x/scale + zero_point).clamp(qmin, qmax).round().byte()

    return QTensor(result, scale, zero_point)
...
def updateStats(x, stats, key) -> Dict[str, Dict[str, float]]:
...
def quantizeLayer(x, layer, stat, scale_x, zp_x) -> Tuple[torch.Tensor, float, float]:
    ...
    '''
    res_x = scale_x * (x + zp_x)

    w = layer.weight.data
    b = layer.bias.data

    # quantize w and b
    w_scale, w_zp = calcScaleZeroPoint(w.min(), w.max())
    b_scale, b_zp = calcScaleZeroPoint(b.min(), b.max())
    w_q = w_scale * (w + w_zp)
    b_q = b_scale * (b + b_zp)

    # replace the w and b in the layer with the quantized w_q and b_q
    layer.weight.data = w_q
    layer.bias.data = b_q

    # do a forward pass and a ReLU
    res_y = F.relu(layer(res_x))

    # restore the original weights
    layer.weight.data = w
    layer.bias.data = b

    # get scale and zero point of res_y
    res_y_scale, res_y_zp = calcScaleZeroPoint(stat['min'], stat['max'])

    return res_y, res_y_scale, res_y_zp
# %%
