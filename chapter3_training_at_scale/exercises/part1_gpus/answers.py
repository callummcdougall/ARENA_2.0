...
# %% No inference mode, CPU, CUDA enabled
...
print(prof.key_averages().table(sort_by="cpu_time_total", row_limit=10))
prof.export_chrome_trace("trace_cpu_only.json")
# %%
model = ResNet34().cuda()
inputs = torch.randn(5, 3, 224, 224).cuda()
...
output = prof.key_averages().table(sort_by="self_cuda_time_total", row_limit=10)
print(output)
prof.export_chrome_trace("trace_cpu_gpu.json")
# %% DAXPY
...
def test_daxpy_random_input(fn1, fn2):
    ...
    alpha = torch.rand(1, device="cuda")
    x = torch.randn(x_val, x_val, device='cuda')
    y = torch.randn(y_val, y_val, device='cuda')

    assert torch.allclose(fn1(alpha, x, y), fn2(alpha, x, y), 0, 1e-100), "Implementations are not analogous"
    print('Tests passed')
...
# %% Same benchmarking with softmax
...
def test_daxpy_random_input(fn1, fn2):
    ...
    x = torch.randn(700, 50000, device='cuda')

    assert torch.allclose(fn1(x), fn2(x), 0, 1e-6), "Implementations are not analogous"
    print('Tests passed')
test_daxpy_random_input(naive_softmax, jit_softmax)


print("benching...")

bench_results = []
for contender in [naive_softmax, jit_softmax]:
    try:
        name = contender.__name__
    except:
        name = contender.name
    ...
    t = benchmark.Timer(
        setup="x = torch.randn(700, 50000, device='cuda')",
        stmt="function(x)",
        description=f"cuda",
        label="softmax",
        sub_label=name,
...
def quantize_tensor(x, min_val=None, max_val=None, num_bits=8) -> QTensor:
    ...
    # get integer zero point
    float_zero_point = - min_val / scale
    approx_zero_point = min(qmax, float_zero_point)
    approx_zero_point = max(qmin, float_zero_point)
    approx_zero_point = int(approx_zero_point)

    # compute q
    q = x / scale + approx_zero_point
    q.clamp_(qmin, qmax)
    q.round_()
    q = q.byte()

    return QTensor(tensor=q, scale=scale, zero_point=approx_zero_point)
...
rand_tensor = torch.rand(5) * 1e-5
print(rand_tensor)

print(dequantize_tensor(quantize_tensor(rand_tensor)))
# %%
