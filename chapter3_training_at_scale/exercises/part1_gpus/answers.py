...
def quantize_tensor(x, min_val=None, max_val=None, num_bits=8) -> QTensor:
    ...
    q_x = q_x.byte()
    # q_tensor = torch.round((x/scale) + zero_point).byte()

    return QTensor(q_x, scale, zero_point) 
...
def updateStats(x, stats, key) -> Dict[Dict, typing.Any]:
...
def quantizeLayer(x, layer, stat, scale_x, zp_x) -> Tuple[torch.tensor, float, float]:
    ...
    w_scale, w_zero_point = calcScaleZeroPoint(min_val, max_val)
    
    # Quantize the weights and biases of the layer using the calculated scale and zero point
    weight = layer.weight
    bias = layer.bias
    
    q_weight = weight / w_scale + w_zero_point
    q_bias = bias / w_scale + w_zero_point
    ...
    out_min_val, out_max_val = out.min(), out.max()
    out_scale, out_zero_point = calcScaleZeroPoint(out_min_val, out_max_val)

    return out, out_scale, out_zero_point
# %%
