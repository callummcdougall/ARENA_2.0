...
def main(args):
    ...
    imagenet_valset = list((lambda k=k: read_image(f'./val/{k}.JPEG'), int(v)) for k, v in file_mappings.items())[:100]
    ...
    # dp_dataloader = DataLoader(imagenet_valset, batch_size=32, shuffle=True)
    # losses = []
    # accuracies = []
    # loss_fn = torch.nn.CrossEntropyLoss()
    # for batch,label in dp_dataloader:

    #     # batch.to('cuda:'+str(0 if UNIGPU else rank))
    #     # print(batch.shape)
    #     # print(label.shape)
    #     # print(batch.device)

    #     cuda_label = label.to('cuda:'+str(0 if UNIGPU else rank))
    #     inference = resnet34(batch.to('cuda:'+str(0 if UNIGPU else rank)))
    #     batch_loss = loss_fn(inference, cuda_label)
    #     dist.reduce(batch_loss, 0, op=ReduceOp.AVG)
    #     batch_accuracy = (torch.argmax(inference, dim=-1) == cuda_label).bool().float()
    #     # print(batch_accuracy.dtype)

    #     dist.reduce(batch_accuracy, 0, op=ReduceOp.AVG)
    #     print(batch_loss.shape)
    #     print(batch_accuracy.shape)
    #     losses.append( batch_loss.mean().item())
    #     accuracies.append( batch_accuracy.mean().item() )
    # if rank == 0:
